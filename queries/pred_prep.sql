...
cte_tomorrow_game_log AS (
    ...
    INNER JOIN (    
        SELECT slug_season, LEFT(SLUG_MATCHUP, 3) AS team_slug, game_date, game_id FROM nba.league_game_schedule WHERE game_date = (SELECT MAX(game_date) + 1 FROM cte_prior_game_log)
        UNION ALL
        SELECT slug_season, RIGHT(SLUG_MATCHUP, 3) AS team_slug, game_date, game_id FROM nba.league_game_schedule WHERE game_date = (SELECT MAX(game_date) + 1 FROM cte_prior_game_log)
